...
const items = [
  ...
  ["Payments", "/admin/payments", ReceiptText],
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-72 bg-[#071426] p-5 text-white lg:block">
        ...
        <nav className="mt-8 space-y-1">
          {items.map(([label, href, Icon]) => (
            <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium", pathname === href || (href !== "/admin" && pathname.startsWith(`${href}/`)) ? "bg-blue-600 text-white" : "text-slate-300 hover:bg-white/10 hover:text-white")}>
